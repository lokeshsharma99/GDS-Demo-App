...
export const validateStep = (step: number, formData: FormData): FormErrors => {
  ...
  if (step === 1) {
    ...
    if (formData.firstName.trim().length > 70) {
      errors.firstName = 'First name must be 70 characters or fewer';
    }

    if (formData.lastName.trim().length > 70) {
      errors.lastName = 'Last name must be 70 characters or fewer';
    }

    // Date of birth — three separate fields (optional, but validate if any part is provided)
  ...
  if (step === 2) {
    ...
    if (formData.phone.trim() && !/^(\+44\s?|0)[\d\s]{9,14}$/.test(formData.phone.trim())) {
      errors.phone = 'Enter a telephone number, like 01632 960 001, 07700 900 982 or +44 808 157 0192';
    }

    // UK postcode: validate format only if a value is provided
    if (formData.postcode.trim()) {
      const postcodeNormalised = formData.postcode.replace(/\s/g, '').toUpperCase();
      if (!/^[A-Z]{1,2}\d[A-Z\d]?\d[A-Z]{2}$/i.test(postcodeNormalised)) {
